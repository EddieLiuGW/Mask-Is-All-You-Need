...
static void hybrid_mpc_sbox_s(const uint32_t* in, const uint32_t* out, randomTape_t* tapes, msgs_t* msgs, uint32_t* state, uint32_t tapes_pos, uint32_t msgs_pos, paramset_t* params, int r)
{
    ...
    for (size_t i = 0; i < params->numSboxes * 3; i += 3) {
        ...
        uint8_t lambda_d = getBitFromWordArray(out, i + 2);
        uint8_t lambda_e = getBitFromWordArray(out, i + 1);
        uint8_t lambda_f = getBitFromWordArray(out, i);


        uint8_t fresh_output_mask_ab = lambda_f ^ lambda_a ^ lambda_b ^ lambda_c;
        uint8_t fresh_output_mask_bc = lambda_d ^ lambda_a;
...
static void computeHybridPhase(randomTape_t* tapes, uint8_t* inputs, msgs_t* msgs, uint32_t* state, paramset_t* params)
{
    ...
    tapes->pos = 0;
    msgs->pos = params->numRounds * params->stateSizeBits;
    ...
    size_t messageByteLength, signature2_t* sig, paramset_t* params)
{
    ...
    /* LowMC */
    uint32_t* state = malloc(params->numRounds * params->stateSizeWords * (sizeof(uint32_t))); //free
    LowMCEncState(plaintext, privateKey, params, state);

    /* Preprocessing; compute aux tape for the N-th player, for each parallel rep */   //1
    inputs_t inputs = allocateInputs(params);
    uint8_t auxBits[MAX_AUX_BYTES] = { 0, };
    msgs_t* msgs = allocateMsgs(params);
    for (size_t t = 0; t < params->numMPCRounds; t++) {
        computeHybridPhase(&tapes[t], inputs[t], &msgs[t], state, params);
